 ...
 */
$seFileName = __DIR__ . "/" . $GLOBALS['dataDir'] . "/service_endpoints.xml";
...
foreach ($ses as $xmlSe) {
    ...
    // Set the scope
    if ((string) $xmlSe->SCOPES->SCOPE == "EGI") {
        $doctrineSe->addScope($egiScope);
    } else if ((String) $xmlSe->SCOPES->SCOPE == 'Local') {
        $doctrineSe->addScope($localScope);
    } else {
        throw new Exception("Unknown scope " . $xmlSe->SCOPES->SCOPE . " for SE " . $xmlSe->HOSTNAME);
    }
    ...
    $xmlEndpoint = $xmlSe->ENDPOINTS->ENDPOINT;
    $doctrineEndpointLocation->setUrl((string) $xmlEndpoint->URL);
    $doctrineEndpointLocation->setName((string) $xmlEndpoint->NAME);
    $doctrineEndpointLocation->setInterfaceName((string) $xmlEndpoint->INTERFACENAME);
