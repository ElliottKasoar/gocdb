...
$xmlCertStatusChanges = simplexml_load_file( __DIR__ . "/" . $GLOBALS['dataDir'] . "/cert_status_changes.xml");
$xmlCertStatusLinkDates = simplexml_load_file(__DIR__ . "/" . $GLOBALS['dataDir'] . "/cert_status_dates.xml");
...
foreach($sites as $xmlSite) {
    ...
    $siteId = (integer) $xmlSite['ID'];

    if ($id>$siteId) {
        throw new Exception("Sites must be ordered by ID");
    }

    while ($siteId!==$id) {
        $doctrineSite->setShortName("Delete" . (string) $id);
        $entityManager->persist($doctrineSite);
        $entityManager->flush();
        $entityManager->remove($doctrineSite);
        $doctrineSite = new Site();
        $id++;
    }
    ...
    //$doctrineSite->setId((integer) $xmlSite['ID']);
    //$doctrineSite->setId((integer) 21);
    //$entityManager->persist($doctrineSite);
    //throw new Exception($doctrineSite->getId() . " TESTING " . $xmlSite['ID']);

    //$metadata = $entityManager->getClassMetaData(get_class($doctrineSite));
    ...
    $doctrineSite->setIpRange((string) $xmlSite->SITE_IP);
    ...
    foreach($xmlCertStatusLinkDates as $xmlCertStatusLinkDate){
       ...
       if($targetSiteName == $doctrineSite->getShortName()){
          ...
          $linkDate =  \DateTime::createFromFormat('d-M-y H.i.s+', $xmlLinkDateString, new \DateTimeZone('UTC'));
          if(!$linkDate) {
